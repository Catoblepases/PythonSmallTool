...
for filename in files:
    ...
    def cut2(line):
        begin, end = 0, 0
        PTL=["PRINTFORML ","RETURN_STR '=  ","DATAFORM ", "PRINTFORMDW ","PRINTFORMDL ","PRINTFORMW "]
        PT=""
        ...
        if((PT=="") or ((PT==PTL[1]) and (line[begin] in "QWERTYUIOPASDFGHJKLZXCVBNM")) or (line.count('%')>=6)):
            return 0,0
    ...
    for i in range(len(to_translate)):
        ...
        if(not TOGETHOR[i]):
            TEXT[i]=translator.translate(to_translate[i],src='ja',dest='zh')
            if(type(TEXT[i])!=str):
    ...
    for i in range(len(be)):
        ...
        line=lines[i]
        if(not TOGETHOR[i]):
            newline = line[:begin+1]+TEXT[i]+line[end:]
        else:
            newline=line
        document += newline+"\n"
